...
(function() {
	...
	*/
	...
	var APP_ROOT_ID = "fileworkflow-chatgpt-queue-root";
	var PANEL_ID = "fileworkflow-chatgpt-queue-panel";
	var STYLE_ID = "fileworkflow-chatgpt-queue-style";
	var RANDOM_WAIT = {
		minMinutes: 1,
		maxMinutes: 10
	};
	...
	//#endregion
	//#region src/hooks/usePanelPosition.ts
	...
	function usePanelPosition(panelRef, headerRef) {
		const getPanelTravel = (panel) => {
			const rect = panel.getBoundingClientRect();
			const viewport = viewportSize();
			return {
				rect,
				viewport,
				maxLeft: Math.max(8, viewport.width - rect.width - 8),
				maxTop: Math.max(8, viewport.height - rect.height - 8)
			};
		};
		const clampPanelPosition = (panel, left, top) => {
			const travel = getPanelTravel(panel);
			return {
				left: clamp(left, 8, travel.maxLeft),
				top: clamp(top, 8, travel.maxTop)
			};
		};
		const savePanelPosition = (panel, left, top) => {
			const next = clampPanelPosition(panel, left, top);
			const travel = getPanelTravel(panel);
			const xRange = Math.max(1, travel.maxLeft - 8);
			const yRange = Math.max(1, travel.maxTop - 8);
			safeJsonSet(STORAGE_KEYS.panelPosition, {
				left: next.left,
				top: next.top,
				leftRatio: clamp((next.left - 8) / xRange, 0, 1),
				topRatio: clamp((next.top - 8) / yRange, 0, 1)
			});
		};
		const placePanel = (panel, left, top, shouldSave = false) => {
			const next = clampPanelPosition(panel, left, top);
		...
		const placePanelBySaved = (panel, saved, shouldSave = false) => {
			if (saved && Number.isFinite(saved.leftRatio) && Number.isFinite(saved.topRatio)) {
				const travel = getPanelTravel(panel);
				placePanel(panel, 8 + Number(saved.leftRatio) * Math.max(1, travel.maxLeft - 8), 8 + Number(saved.topRatio) * Math.max(1, travel.maxTop - 8), shouldSave);
				return;
			}
		...
		(0, import_react.useEffect)(() => {
			...
			requestAnimationFrame(() => placePanelBySaved(panel, safeJsonGet(STORAGE_KEYS.panelPosition, null), true));
			let dragging = false;
			let pointerId = null;
			let startX = 0;
			let startY = 0;
			...
			const onResize = () => placePanelBySaved(panel, safeJsonGet(STORAGE_KEYS.panelPosition, null), false);
	...
	//#endregion
	//#region src/shared/format.ts
	function normalizeText(text) {
		return String(text || "").replace(/\u200b/g, "").replace(/[ \t]+/g, " ").trim();
	}
	function normalizePromptUnit(text) {
	...
	//#endregion
	//#region src/features/queue/completionSignal.ts
	function isCompletionSignal(text) {
		...
		return candidates.some((item) => {
			const compact = normalizeText(item).replace(/\s+/g, "").replace(/[“”"'`‘’【】\[\]（）(){}<>《》、，,。.:：!！?？;；~～—\-]/g, "");
	...
	//#endregion
	//#region src/features/queue/promptText.ts
	var DEFAULT_PROMPT_TEXT = [
		"第一条消息：请总结 TCP 三次握手。",
		"第二条消息：请总结 TCP 四次挥手。",
		"第三条消息：请对比 TCP 和 UDP。"
	].join("\n");
	function getSavedPromptText() {
	...
	//#endregion
	//#region src/features/queue/parser.ts
	function getLogicalQueueLines(raw) {
	...
	function parsePrompts(raw) {
		...
		if (!text) return [];
		if (/^\s*(?:---+|===+)\s*$/m.test(text)) return text.split(/^\s*(?:---+|===+)\s*$/m).map((item) => item.trim()).filter(Boolean);
		return getLogicalQueueLines(text).map((item) => item.trim()).filter(Boolean);
	}
	function getUrlCommand(text) {
		const match = String(text || "").replace(/\s+/g, "").trim().match(/^<((?:https:\/\/)[^<>]+)>$/i);
		if (!match) return "";
		try {
	...
	function getQueueFingerprint(raw) {
		return parsePrompts(raw).map(normalizePromptUnit).filter(Boolean).join("\n---\n");
	}
	//#endregion
  ...
  `;
	...
	async function sleepInterruptible(ms, shouldStop, stopMessage = "已停止", step = 1e3) {
	...
	//#endregion
	//#region src/services/chatgpt/domAdapter.ts
	function isFromPanel(el) {
	...
	function visibleOne(selectors, root = document) {
	...
	function getSendButton() {
		const composer = getComposer();
		const roots = composer ? [composer.closest("form"), document].filter(Boolean) : [document];
		const selectors = [
		...
		for (const root of roots) {
			const button = visibleOne(selectors, root);
	...
	function getAssistantMessages() {
		return Array.from(document.querySelectorAll("[data-message-author-role=\"assistant\"]")).filter((el) => !isFromPanel(el) && document.contains(el));
	}
	function getAssistantCount() {
		return getAssistantMessages().length;
	}
	function getAssistantContentText(message) {
		if (!message) return "";
		const clone = (message.querySelector(".markdown, .prose, [data-message-id]") || message).cloneNode(true);
	...
	function hasBusyIndicator() {
		return [getAssistantMessages().pop(), document.querySelector("main")].filter(Boolean).some((root) => Boolean(root.querySelector("[aria-busy=\"true\"], [data-testid*=\"loading\"], .result-streaming")));
	}
	...
	//#endregion
	//#region src/services/chatgpt/composer.ts
	...
	//#endregion
	//#region src/hooks/useQueueController.ts
	var initialRunnerState = {
		running: false,
		paused: false,
	...
	function useQueueController() {
		const [promptText, setPromptTextInner] = (0, import_react.useState)(getSavedPromptText);
		const [runnerState, setRunnerState] = (0, import_react.useState)(() => ({
			...initialRunnerState,
			prompts: parsePrompts(getSavedPromptText())
		}));
		const [settings, setSettingsInner] = (0, import_react.useState)(() => ({
			autoScroll: safeGet(STORAGE_KEYS.autoScroll, "true") !== "false",
			randomWait: safeGet(STORAGE_KEYS.randomWait, "false") === "true"
		}));
		const [now, setNow] = (0, import_react.useState)(Date.now());
		const savedFingerprintRef = (0, import_react.useRef)(getQueueFingerprint(promptText));
		const runnerRef = (0, import_react.useRef)(runnerState);
		const forceResolveRef = (0, import_react.useRef)(null);
		(0, import_react.useEffect)(() => {
		...
		const patchRunner = (0, import_react.useCallback)((patch) => {
			runnerRef.current = {
				...runnerRef.current,
				...patch
			};
		...
		const setPromptText = (0, import_react.useCallback)((value) => {
			setPromptTextInner(value);
			patchRunner({ inputDirty: getQueueFingerprint(value) !== savedFingerprintRef.current });
		}, [patchRunner]);
		const setSettings = (0, import_react.useCallback)((patch) => {
		...
		const applyInputQueue = (0, import_react.useCallback)((options = {}) => {
		...
		const saveNavigationRunState = (0, import_react.useCallback)((nextIndex) => {
		...
		const clearNavigationRunState = (0, import_react.useCallback)(() => safeRemove(STORAGE_KEYS.runState), []);
		const endAndReset = (0, import_react.useCallback)((status = "已结束") => {
		...
		const sendPrompt = (0, import_react.useCallback)(async (text) => {
		...
		const waitForReplyDone = (0, import_react.useCallback)(async (snapshot) => {
			const start = Date.now();
			...
			let cancelled = false;
			const manualDone = new Promise((resolve) => {
				forceResolveRef.current = resolve;
			});
			const autoDone = (async () => {
				while (!cancelled && Date.now() - snapshot.sentAt < 6e5) {
					if (runnerRef.current.endRequested) throw new Error("已结束队列");
					...
					controlsIdleSince = !isGenerating() && (isComposerReady() || Boolean(getSendButton())) ? controlsIdleSince || currentTime : 0;
					const minWaitPassed = currentTime - snapshot.sentAt > 3e3;
					...
					const fallbackDone = controlsStable && currentTime - snapshot.sentAt > 9e3;
		...
		const runQueue = (0, import_react.useCallback)(async () => {
			if (runnerRef.current.running) return;
			const prompts = applyInputQueue({
				persist: true,
				silent: true
			});
			if (!prompts.length) {
			...
			try {
				while (runnerRef.current.index < runnerRef.current.prompts.length) {
					...
					if (runnerRef.current.skipUntilUrl) {
						patchRunner({
							index: currentIndex + 1,
							status: `当前任务已完成，跳过普通消息 ${currentIndex + 1}/${runnerRef.current.prompts.length}`
						});
						await sleep(80);
						continue;
					}
					...
					if (getLastAssistantText() !== snapshot.baseText && isCompletionSignal(getLastAssistantText())) {
						patchRunner({
							completedByEndSignal: true,
							skipUntilUrl: true,
							status: "检测到当前任务完成，跳过后续普通消息，等待下一个网页"
						});
						continue;
					}
				...
				if (runnerRef.current.pauseRequested) {
					patchRunner({
						...
						pauseRequested: false,
						finishedAt: Date.now(),
						status: `已暂停 ${runnerRef.current.index}/${runnerRef.current.prompts.length}`
					});
					return;
				}
				clearNavigationRunState();
				patchRunner({
					...
					finishedAt: Date.now(),
					skipUntilUrl: false,
					status: runnerRef.current.index >= runnerRef.current.prompts.length ? "完成" : "已停止"
				});
			} catch (error) {
			...
			endAndReset,
			patchRunner,
			saveNavigationRunState,
			sendPrompt,
			settings.randomWait,
			waitForReplyDone
		]);
		const pauseOrEndQueue = (0, import_react.useCallback)(() => {
			...
			patchRunner({
				pauseRequested: true,
				status: state.waitingUntil ? "正在暂停：随机等待已取消" : "正在暂停：当前回复完成后停止继续发送"
			});
		}, [endAndReset, patchRunner]);
		const forceFinish = (0, import_react.useCallback)(() => {
			forceResolveRef.current?.();
			patchRunner({ status: "已手动结束当前等待" });
		}, [patchRunner]);
		function resumePendingRunIfNeeded() {
			const saved = safeJsonGet(STORAGE_KEYS.runState, null);
			if (!saved?.active) return;
			if (saved.savedAt && Date.now() - saved.savedAt > 360 * 60 * 1e3) {
				clearNavigationRunState();
				return;
			}
		...
		return {
			...
			view: (0, import_react.useMemo)(() => {
				...
				const current = runnerState.running || runnerState.paused ? Math.min(runnerState.index + (runnerState.running && !runnerState.waitingUntil ? 1 : 0), total) : Math.min(runnerState.index, total);
				const elapsed = runnerState.startedAt ? (runnerState.finishedAt || now) - runnerState.startedAt : 0;
				const mode = runnerState.endRequested ? "ending" : runnerState.pauseRequested ? "pausing" : runnerState.waitingUntil ? "waiting" : runnerState.running ? "running" : runnerState.paused ? "paused" : /错|异常|失败|超时/.test(runnerState.status) ? "error" : /完成|完毕/.test(runnerState.status) ? "done" : "idle";
				const shortStatus = mode === "ending" ? "结束" : mode === "pausing" ? "暂停" : mode === "waiting" ? "等待" : mode === "running" ? "运行" : mode === "paused" ? "暂停" : mode === "error" ? "错误" : mode === "done" ? "完成" : /保存/.test(runnerState.status) ? "保存" : "空闲";
				const detail = runnerState.waitingUntil ? `随机等待中：剩余 ${formatDuration(Math.max(0, runnerState.waitingUntil - now))}` : runnerState.status.length > 42 ? `${runnerState.status.slice(0, 42)}…` : runnerState.status;
				return {
					mode,
					total,
					current,
	...
	*/
	...
	//#endregion
	//#region src/components/PromptEditor.tsx
	...
	function renderPromptHighlight(text) {
	...
	function PromptEditor({ value, lineCount, onChange }) {
		const [scroll, setScroll] = (0, import_react.useState)({
			x: 0,
			y: 0
		});
		const highlighted = (0, import_react.useMemo)(() => renderPromptHighlight(value), [value]);
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "fw-pq-field",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
				className: "fw-pq-label",
				htmlFor: "fileworkflow-chatgpt-queue-input",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: "消息队列" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
					"共 ",
					lineCount,
					" 行"
				] })]
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("pre", {
					className: "fw-pq-input-highlight",
					"aria-hidden": "true",
	...
	//#endregion
	//#region src/components/StatsGrid.tsx
	function StatsGrid({ total, current, elapsedText, shortStatus }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "fw-pq-stats",
			role: "status",
			"aria-live": "polite",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "fw-pq-stat",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "总数" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: total })]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "fw-pq-stat",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "当前" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("b", { children: [
						current,
						"/",
						total
					] })]
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "fw-pq-stat",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "用时" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: elapsedText })]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "fw-pq-stat",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "状态" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: shortStatus })]
				})
			]
		});
	}
	...
	//#endregion
	//#region src/components/FloatingPanel.tsx
	function FloatingPanel() {
		const panelRef = (0, import_react.useRef)(null);
		const headerRef = (0, import_react.useRef)(null);
		const [collapsed, setCollapsed] = (0, import_react.useState)(() => safeGet(STORAGE_KEYS.collapsed, "false") === "true");
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				ref: headerRef,
				className: "fw-pq-header",
				"aria-label": "拖动面板",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "fw-pq-title",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "fw-pq-badge",
						"data-mode": queue.view.mode,
						children: badgeText(queue.view.mode)
					}),
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								...
								children: queue.view.startLabel
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								...
								children: queue.view.pauseEndLabel
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								className: "fw-pq-button",
								type: "button",
								disabled: !queue.runnerState.inputDirty,
								onClick: () => queue.actions.applyInputQueue({
									persist: true,
									silent: false
								}),
								children: queue.runnerState.inputDirty ? "保存" : "已保存"
							})
						]
					}),
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "fw-pq-wait-range",
								children: queue.view.waitRange
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "fw-pq-footer",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "fw-pq-detail",
							title: queue.runnerState.status,
							children: queue.view.detail
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
	...
	function App() {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FloatingPanel, {});
	}
	//#endregion
	//#region src/styles/panelStyles.ts
	var panelStyles = `
#${PANEL_ID} {
...
`;
	//#endregion
	//#region src/styles/injectGlobalStyles.ts
	function injectGlobalStyles() {
		if (document.getElementById("fileworkflow-chatgpt-queue-style")) return;
	...
	//#endregion
	//#region src/bootstrap/mountApp.tsx
	function mountFileWorkflowApp() {
		...
		window.__FILEWORKFLOW_CHATGPT_QUEUE__ = {
			...
			unmount: () => {
				root.unmount();
				container?.remove();
				window.__FILEWORKFLOW_CHATGPT_QUEUE__ = void 0;
			}
		};
	}
	function mountFileWorkflowWhenReady() {
		if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", mountFileWorkflowApp, { once: true });
		else mountFileWorkflowApp();
	}
	//#endregion
	//#region src/entry-extension.tsx
	console.info("[FileWorkflow] Chrome extension content script loaded");
